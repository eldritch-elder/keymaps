...
Neither osm nor caps word behave nicely when defining them manually and the alphas aren't shifted with caps word active.
...
(defun get-key-at (pos)
  ...
  (let*
      ((key-def (nth pos (remove "" (flatten-list base))))
       (keycode (parse-key key-def)))
    (pcase key-def
      ((or "&nop" "xxx" "&trns" "---" "___")        (error "the key at pos %d is nop or transparent, abort" pos))
      (-                                            keycode))))
#+end_src
...
(mapcar #'(lambda (x)
            (let ((keydef (pop x)))
              (if (eq "" keydef) (error "a combo can't do nothing"))
              (if (eq "" (car x)) (error "combo `%s` seems to be empty" keydef))))
        in)
...
(while in
  (unless (eq (parse-key (caar in)) "")
    (setq result (concat result " case " (parse-key (car (pop in))) ": "))))
...
#define LAYER_STATE_8BIT // decreases size
...
#define EXTRA_SHORT_COMBOS // decreases size
